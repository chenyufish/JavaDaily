    ...
    public static void LRAnalyse() {
     ...
     Grammar grammar = new Grammar();
        ...
        while (!analyseTable.action[s][analyseTable.getTerminalIndex(ch)].equals("acc")) {
            ...
            // 如果是移进
            if (str.startsWith("s")) {
                System.out.println(String.format("%-10s%-10s%-10s%-10s%-10s", step, vectTrancStr(0), vectTrancStr(1), vectTrancStr(2), "ACTION[" + status.get(status.size() - 1) + "," + ch + "]=S" + s + ", 状态" + s + "入栈"));
                // 输入符号入栈
                sign.add(ch);
                inputStr.remove(0);
                // 将状态数字入栈
            ...
            // 如果是归约
                ...
                System.out.println(String.format("%-10s%-10s%-10s%-10s%-10s", step, vectTrancStr(0), vectTrancStr(1), vectTrancStr(2), "r" + index + ":" + grammar.formula[index] + "归约, GOTO{" + oldStatus + "," + nonTerCh + ")=" + s + "入栈"));
                // 对符号栈进行出栈和状态栈进行出栈
        ...
        System.out.println(String.format("%-10s%-10s%-10s%-10s%-10s", step, vectTrancStr(0), vectTrancStr(1), vectTrancStr(2), "acc:分析成功"));
